...
function Crud() {
  ...
  return (
    <div>
          ...
          {value?.map((item, index) => {
            return (
              <tr>
                ...
                <td>{item.location}</td>
                <td>
                  <FaEdit style={{color:'yellow'}} className="mx-2" type="button" onClick={()=>editData(item)}/>
                  <RiDeleteBin6Fill type="button" style={{color:'red'}} onClick={()=>deleteData(item.id)}/>
                </td>
              </tr>
            );
          })}
      ...
      <Model modalshow={show} modalClose={handleClose} modaldata={temp} modalStatus={setStatus} setModalData={setTemp}/>
    </div>
